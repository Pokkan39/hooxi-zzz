...
function validSiteData(data){return Boolean(data&&isObject(data.home)&&isObject(data.home.appearance)&&Array.isArray(data.home.cards)&&Array.isArray(data.home.tracks)&&isObject(data.archive)&&['mainline','stories','behindScenes','events'].every(key=>Array.isArray(data.archive[key]))&&(!('layout' in data)||isObject(data.layout)))}
