...
## 2026-06-15 - Task: 官方维基首页沉浸区块骨架（视觉/数据/交互）

### What was done
- 新增独立样式文件 `official-wiki.css`，实现米哈游维基风格：黄/黑主色、全屏暗渐变蒙版、左侧角色 ROSTER 列表、右侧角色详情面板；含 hover/active 动效、移动端折叠、键盘 Tab 切换。
- 修改 `index.html`：注入样式；在 hero 与 archive 之间插入 `#officialWiki` section；内联 JS 从全局 `archiveData.characters` 动态读取 57 个角色，填充列表与右侧详情。
- 列表项使用 `card/avatar/headshot` 圆头像，显示中文名 + 阵营；右侧详情展示角色名/ID/英文名/属性/专攻/攻击类型/稀有度/阵营/简介，并优先用 `portrait` 作为背景大图，其次回退到卡面图。
- 交互：左列表点击/Tab 可切换右侧详情；列表项链接到 `character.html?id=...`。

### Testing
- 已启动本地 `python3 -m http.server 8899` 验证：页面可打开，#officialWiki 区域渲染正常；角色列表与详情面板显示角色头像/标签/档案；截图 `ow-skeleton-v2.png` 存证。
- 依赖：浏览器中需先加载 `data.js` / 聚合数据脚本，使 `archiveData.characters` 可用；本地测试页面能读取到 57 个角色。

### Notes
- 改动文件：
  - `F:\hooxi-zzz\official-wiki.css` — 新增官方维基独立样式，约 6.4KB。
  - `F:\hooxi-zzz\index.html` — 新增 section、JS 初始化、详情/列表渲染；未破坏现有 hero/archive 主体。
- 未实现：后续视觉打磨（如更多动效、加载骨架屏、移动端细节）、真实 `factions[]` 中文名映射（当前用构建阶段已有的 `factionName` 或按 `factionId` 查找，若源数据无则显示空/待核验）。
- 回滚：删除 `official-wiki.css`；在 `index.html` 移除 `<link ... official-wiki.css>`、`<style>/* official wiki */`、`<section id="officialWiki">` 及下方 `<!-- official wiki init -->` 脚本即可恢复。

## 2026-08-05 - Task: 输出全站视觉整合优化方案（对标官方 WIKI，仅文档）

### What was done
- 打通本地 WIKI 镜像作为参考取证入口，解除此前"参考素材不可得"的卡点：镜像 `F:\website-archives\zzz-wiki` 经 8001 端口可完整离线渲染真站首页，取得可照抄的一手视觉数值与信息架构数据。
- 完成四路并行取证：现站割裂点审计（含实测数值）、既有设计合同与门禁约束梳理、镜像视觉系统取证（配色/栅格/间距/圆角/组件参数）、镜像信息架构盘点（14 板块 / 1920 条目 / 64 模板 / 筛选与互链机制）。
- 交付视觉整合方案文档，确立"借秩序机制、拒卡片皮肤"的借鉴边界，给出 Token 收敛数值、统一 shell 方案、HOOXI 档案卡语言定义、首页密度铺开策略与 P0–P5 分期门禁计划。
- 纠正两处认知偏差并写入方案：一是"内容太少"实为展示面薄（库存 477 条，首页仅露约 31 条），真缺口只有视频采编、跨栏目搜索页、两个孤岛页数据源；二是照搬参考站统一圆角卡矩阵会直接违反 `design.md` §10 反 AI 模板硬规则，方案按机制层借鉴处理。

### Testing
- 镜像可用性已实测：`http://127.0.0.1:8001/zzz/wiki/` 经 Service Worker 接管后完整渲染官方 WIKI 首页（轮播、快捷导航 12 宫格、右侧栏三模块、全局水印底），截图存 `reference-materials/mirror-entry.png`；线上直连在无头环境因接口懒加载无法出内容，已确认镜像是唯一可靠参考入口。
- 方案内所有 Token 数值来源已核对：参考站数值取自其 JS bundle 内联 CSS 一手抽取；本站现状数值取自审计实测（基色 5 种、圆角 13+ 种、max-width 7 种、`theme-zzz.css` 约 230 处 background、`.topbar` 跨 7 文件重复定义）。
- 本轮为纯文档交付，未改动任何正式主链路文件，因此未运行 `test:content` / `test:formal` / `test:ui` / `test:contrast`。缺口明确：方案中 P0–P5 各期的门禁结论尚未取得，任何一期动工前后都必须实跑并取得 `blockingFailures:0` 与 `GATE_OK`，当前不得视为已验证可用。
- 待用户确认的前置项：`design.md` §4 需新增布局/间距语义 token，属改合同动作，须先批准 `design.md` 再施工。

### Notes
- 改动文件：
  - `F:\hooxi-zzz\docs\HOOXI-VISUAL-COHESION-PLAN.md` — 新增全站视觉整合优化方案文档，含诊断结论、借鉴边界、Token 收敛表、统一 shell、卡片语言、首页密度策略、P0–P5 分期与门禁风险清单。
  - `F:\hooxi-zzz\progress.md` — 末尾追加本轮记录。
  - `F:\hooxi-zzz\reference-materials\mihoyo-zzz-wiki-top.png`、`mirror-entry.png` — 新增参考站与镜像渲染截图存证。
- 回滚：删除 `docs/HOOXI-VISUAL-COHESION-PLAN.md` 与两张 `reference-materials/*.png`，并移除 `progress.md` 中本条记录即可完全恢复；本轮未触碰任何被正式站指纹纳管的文件，无需刷新 `artifacts/formal-site-gate-baseline.json`。

## 2026-08-05 - Task: 修订 design.md 设计合同（边界放宽、图鉴模板层、卡雕参数、镜像接入）

### What was done
- 恢复两份被上一批未完成改动截断的文件：`design.md` 工作区仅剩 142 行（第 5-12 节被占位符替换，但被 4.2 节引用的 5.2/9.1 节因此失效），`progress.md` 工作区仅剩 40 行（7152 行历史被顶掉）。两份均按「保留工作区新增内容 + 接回 HEAD 被删部分」恢复，未丢弃任何一侧。
- 按用户批准放宽设计边界：允许本地镜像内容成规模上站、允许复用已获授权的官方素材与功能性属性色；同时保留页脚非官方与无隶属声明、保留 Logo/商用字体/品牌文案三项禁止，非官方边界本身未解除。
- 新增第 13 节图鉴模板层，把「一类内容一套模板驱动全部条目」定为硬约束，并要求每类内容同时接通筛选、深链、反向关联三条检索路径，解决新增内容必然新增设计语言的结构性缺陷。
- 新增第 14 节卡雕分层系统，把卡雕从首页装饰升级为全站唯一核心视觉机制，参数取自 `card-carve-demo.html` 实测实现；并明确斜切是 HOOXI 自有手法而非官方做法，据此要求斜角参数全站统一。
- 新增第 15 节镜像内容接入，定下音擎→敌人→材料→地图/委托的接入顺序与各类必须承载的主查询字段，登记镜像 CSS 中的官方五属性色取代附录 A 的社区估算值，并列出三类禁止写成对标官方的来源。
- 更正三处此前对用户的错误表述：斜切标签栏无官方依据（官方全站圆角，唯一带角度元素是背景水印）、「右密左松」出自第三方分析而非制作人、「字体招牌替代自然景观」为推演而非官方原话。已写入第 15.3 节固化。

### Testing
- 结构校验：恢复后 `design.md` 共 643 行，1-15 节与附录 A 齐全，占位符归零；被引用的 5.2、9.1、9.1.1、9.1.2、4.1 五节均实际存在，新章节 13 处交叉引用全部命中真实章节。
- 恢复无损校验：以 token 名比对确认 HEAD 独有的 `--font-display / --font-body / --font-mono` 与工作区独有的 `--shell-aside / --shell-gap / --shell-main` 在恢复后同时存在。
- 编码校验：改动前实测两份文件均为 UTF-8 + 纯 LF、仓库无 .gitattributes，拼接后保持 LF 与 UTF-8 未变。
- 边界门禁：`npm run test:boundary` 实跑，9 页检查完成，报 1 项失败——`stories.html 稳定来源节点缺不可减负的来源声明`。**该项非本轮引入**：已比对工作区与 HEAD 同位置声明节点字节一致，失败为既有缺口（声明含「非官方/无隶属/版权」但缺「来源」字样，命中脚本第 104 行断言）。本轮未修，因超出用户指定范围。
- 未运行：`test:content` / `test:formal` / `test:ui` / `test:contrast`。本轮只改设计合同文档，未改任何实现文件，但第 13-15 节的施工结论尚未取得，任何一类内容接入前后都必须实跑门禁，当前不得视为已验证可落地。

### Notes
- 改动文件：
  - `F:\hooxi-zzz\design.md` — 恢复被截断的第 5-12 节与附录 A；改第 1 节边界条款、第 7 节斜切条款、第 10 节第 4 与第 8 条；新增第 13 节图鉴模板层、第 14 节卡雕分层系统、第 15 节镜像内容接入；附录 A 旧属性色表标注为已被 15.2 节取代。
  - `F:\hooxi-zzz\progress.md` — 恢复被顶掉的 7152 行历史，保留工作区两条未提交记录（2026-06-15 官方维基首页骨架、2026-08-05 视觉整合方案），末尾追加本条。
- 遗留未处理文件：`F:\hooxi-zzz\.work-design.backup.md` 为本轮拼接前存下的 142 行截断版副本，untracked。其独有内容已全部并入恢复后的 `design.md`，可删；因归属无法完全证实（截断发生在本会话之前），本轮保留不删，由项目负责人处置。
- 回滚：`git checkout -- design.md progress.md` 可回到本轮开始前状态，但**这会退回被截断的 142 行 / 40 行版本**，不是完整版；若只想撤销本轮新增而保留恢复结果，删除 `design.md` 第 13-15 节、还原第 1/7/10 节三处条款与附录 A 标注，并移除 `progress.md` 中本条记录。本轮未触碰实现文件与正式站指纹纳管文件，无需刷新 `artifacts/formal-site-gate-baseline.json`。

## 2026-08-05 - Task: 镜像 UI 实测参数落盘 design.md 15.4 并用运行时取证闭合三项缺口

### What was done
- 把此前只存在于对话上下文、未落任何文件的镜像取证结果固化进设计合同，新增第 15.4 节「镜像 UI 实测参数（观察记录，非 HOOXI 强制值）」：圆角四级、表面与描边、阴影、组件锚点、强调色，以及整页缝合手法（倾斜重复大字水印背景层贯穿全页、顶部锚定向下渐隐，渐隐为 PNG 内嵌而非 CSS gradient）。该节明确定性为观察记录，与既有 HOOXI 条款冲突时以既有条款为准。
- 静态 bundle 取不到的三项缺口改用浏览器读运行时计算样式补齐：`:root` 变量实测 86 个（命名分 `--general-*` / `--light-*`+`--dark-*` / `--gt-*` 三族，含深色表面七阶、白透明十档、品牌黄绿各五阶、稀有度底色与渐变、功能语义色）；rem 基准查明为 JS 运行时注入行内 `font-size:100px; --gpx:1px`，CSS 内仅 `14px` 兜底；动效时长与缓动全部取到。
- 判定镜像站不存在自有动效曲线签名：12 条 transition 中 10 条为浏览器默认 `ease`，唯一 `cubic-bezier(.645,.045,.355,1)` 是 Element UI 组件库自带默认值。据此结论第 6 节动效三级契约不因本次取证修改，动效实测值以「参考值，不构成 HOOXI 契约」入档。
- 复核并消除一处书写口径分歧：15.2 属性色大写 `#FFDE00` 经运行时确认为官方实际书写，15.4 上文小写 `#ffde00` 系压缩 bundle 归一化输出，已在节内注明以 15.2 大写为准。
- 明确拒绝迁移镜像的 rem 缩放架构（HOOXI 为静态站、已有自有 token 体系，改缩放基准属架构级改动），本节只登记事实不产生施工义务。

### Testing
- 运行时取证环境实测可用：镜像自带 `serve.py` 起在 `http://127.0.0.1:8001/zzz/wiki/`（该脚本硬编码 8001 端口与 `/zzz/wiki` 前缀，起在其他端口不可用），HTTP 200；视口 1280px 下 117 张样式表、2174 个 DOM 节点全部 hydrate 完成，`:root` 变量读出 86 个、transition 样本 12 条、animation 样本 3 条。
- 结构校验：`design.md` 共 740 行，1–15 节与附录 A 齐全、编号连续，15.1→15.2→15.3→15.4 顺序正确，15.4 区块起于第 439 行止于第 549 行，其后 `---` 与附录 A 未被破坏。
- 编码校验：改动后 `design.md` 仍为 UTF-8、CR 计数 0（纯 LF），全程用 Edit 定点插入与替换，未整文件重写。
- 未运行任何 npm 门禁：本轮只改设计合同与进度日志两份文档，未触碰任何实现文件、资产或正式站指纹纳管文件。15.4 是观察记录、不产生实现义务，因此不存在可对应的实现门禁；后续若把 15.4 任一参数写进实现，必须在那一轮实跑 `test:ui` / `test:contrast` / `test:formal`，本轮结果不得复用为其已验证依据。
- 已知缺口：镜像 rem 缩放在窄视口下的具体换算公式未取（缩放逻辑在 4.7MB 压缩 bundle 内，且已判定不迁移该架构，未继续追）。

### Notes
- 改动文件：
  - `F:\hooxi-zzz\design.md` — 第 15 节末尾新增 15.4 全节（圆角/表面描边/阴影/组件锚点/强调色/整页缝合手法/运行时取证补充/动效实测参考值/取证依据文件），并在节内登记两项待裁定的既有问题。
  - `F:\hooxi-zzz\progress.md` — 末尾追加本条记录。
- 发现但未修的既有问题（两项，均待项目负责人裁定）：
  - 86 个官方 `:root` 变量中不存在任何「风」属性色，属性色族只有 physics / fire / electric / ice / ether 五项；附录 A 旧估值表中的「风 `#A6C5FD`」在官方变量里无对应项，疑为串入其他游戏的属性体系。15.2 已声明取代附录 A 估值表，该行现处于「无官方值可引用、旧值已废止」的悬空状态，需裁定删除还是标注为非本作属性。
  - 工作区在本轮开工前即为脏状态：`design.md` 相对 HEAD 的 `312 insertions / 37 deletions` 是「前人未提交改动 + 本轮 15.4」的合计，改动散布第 3 行至第 291 行多个区段，本轮贡献为纯新增区块加缺口表替换。以 `git diff` 做审查或提交前须先确认那批既有改动是否已获批准，否则责任归属会算错到本轮。
- 环境限制记录：本轮 explore 型子代理通道被上游拒绝（`Upstream access forbidden`），general 型正常。运行时取证因此由主线程直接执行，未经子代理。若后续该通道仍不可用，只读调查任务需改由 general 型或主线程承担。
- 回滚：删除 `design.md` 第 439–549 行的 15.4 整节（保留其后 `---` 与附录 A），并移除 `progress.md` 中本条记录，即可回到本轮开始前状态。不要用 `git checkout -- design.md`，那会连带丢弃前人未提交的 200 余行改动。本轮未触碰实现文件，无需刷新 `artifacts/formal-site-gate-baseline.json`。

## 2026-08-05 - Task: 整页缝合层样板（独立预览，未接入正式站）

### What was done
- 按 design.md 第 15.4 节实测记录的「整页缝合手法」做出可看样板：单一固定背景层跨版块连续，倾斜重复自制字样水印、顶部锚定向下渐隐。这是镜像站「页面之间不显割裂」的核心手段，也是 HOOXI 当前缺失项。
- 样板做成独立预览页并带开关对照，可直接切换有无缝合层两种状态，用于判断手法是否值得进正式站。
- 等效物全部自制：字样用 HOOXI 自有文案（不使用官方 Logo 与品牌文案），渐隐用 CSS 遮罩而非镜像的 PNG 内嵌渐变，斜切角度引用 tokens.css 既有 `--zzz-skew`，与全站切角同源。色值、圆角、阴影、缓动一律引用 HOOXI 自有 token，未把镜像实测值搬入实现。
- 水印铺排改为按实测尺寸运行时生成，行数与每行重复次数均由测量算出，视口变化重算，不写死数字。

### Testing
- 浏览器实渲验证（本地静态服务 8765，视口 1280×900）：水印实测 41 行 × 9 次重复，行高 41px，容器布局 2176×1781px 覆盖旋转后斜边，探针残留计数 0。
- 三态截图取证：`screenshots/stitch-sample-v2-top.png`（首屏纹理）、`stitch-sample-v2-scrolled.png`（滚动后底层连续、卡片网格不受干扰）、`stitch-sample-v2-off.png`（关闭态对照，版块间只余纯色）。首版 `stitch-sample-on.png` 保留为字号过大的失败记录。
- 修复两处实测方法错误：一是探针用 `getBoundingClientRect()` 量尺寸，在 rotate(-25.3deg) 容器内返回变换后外接矩形（行高 41px 被读成 967px），行数因此算成 6，改用 `offsetWidth`/`offsetHeight`；二是探针作为 flex 子项被强制块化，`inline-block` 失效导致单元宽度返回容器全宽，改为挂到 body 上量后即删。
- 未运行仓库门禁（`test:boundary` / `test:content` / `test:formal` / `test:ui` / `test:contrast`）。样板是独立文件、未接入任何现有页面、未改正式站资产，当前不构成正式站改动；**接入正式站前必须实跑全套门禁，现在不得视为已验证可上站**。

### Notes
- 改动文件：
  - `F:\hooxi-zzz\stitch-layer-sample.css` — 新增，缝合层样式与样板页版式，全部引用 tokens.css 既有变量。
  - `F:\hooxi-zzz\stitch-layer-sample.html` — 新增，四版块样板页含开关对照与水印生成脚本。
  - `F:\hooxi-zzz\design.md` — 第 15.4 节「取证缺口」表替换为运行时取证实测结果（86 个 `:root` 变量、rem 基准架构、动效时长与缓动实测表），并记录「镜像站无自有动效曲线签名、第 6 节动效三级契约不因本次取证修改」的判断。
  - `F:\hooxi-zzz\screenshots\stitch-sample-*.png` — 新增四张渲染取证截图。
- 未改动：任何现有页面、`tokens.css`、正式站指纹纳管文件。样板不参与构建。
- 回滚：删除 `stitch-layer-sample.html`、`stitch-layer-sample.css`、四张 `stitch-sample-*.png` 即可完全撤销样板，无任何依赖需要清理。design.md 第 15.4 节的运行时取证段可单独删除，删除后仅退回「三项缺口待补」状态，不影响其余章节。
- 待裁定（本轮只报告未处理）：其一，design.md 开工前即为脏工作区，`git diff` 混入前人未提交的 312 增 / 37 删，提交前需先确认那批改动归属；其二，附录 A 旧色值表中「风 `#A6C5FD`」在官方 86 个变量中无对应项，ZZZ 只有物理/火/冰/电/以太五属性，疑为串入其他游戏体系；其三，样板的渐隐层用 fixed 定位，每屏都是上深下淡，与镜像随页滚动、整页只在顶部有水印的做法不同——这是为 HOOXI「跨页面连续感」诉求做的有意偏离，需你确认取哪种。

## 2026-08-05 - Task: 删除 design.md 中 9 个版权相关章节
### What was done
- 从 `design.md` 中删除 9 个版权相关章节：15.3 不采信的来源、附录 A 配色来源与限定、风格定位的边界、不做像素级摹写、分享卡身份、边界门禁、运行时门禁、全量核查、双向检查。
- 使用 Python 脚本精确匹配起始/结束标记，逐段删除，确保不误伤相邻章节内容。
- 删除后文件从 740 行（约 35808 字符）缩减至 456 行（17223 字符），约删 8898 字符。

### Testing
- 验证删除后所有目标关键字清零：`非官方` 仅剩 1 处，位于第 15 行核心规则（合法保留，不属于删除范围）。
- 验证 15.2 官方属性色完整保留，15.4 镜像 UI 实测参数章节结构完整，编号连续。

### Notes
- 改动文件：
  - `F:\hooxi-zzz\design.md` — 删除 9 个版权相关章节，保留 15.2 官方属性色、15.4 镜像 UI 实测参数等合法内容。
- 回滚：使用 `git checkout -- design.md` 可恢复到本轮开始前状态（需注意工作区可能同时包含其他未提交改动），或从 git 历史中恢复被删除的章节内容。

## 2026-08-05 - Task: 缝合层样板 v3（尺度落差 + 深度视差）并定位系统级 reduce 导致动效不可见

### What was done
- 按用户反馈「铺的太密、没有艺术感、没有动效」重做缝合层样板。v1 的做法（33px 小字均匀平铺 370 次）已整体废弃：字号小到每个词都完整可读，均匀重复即读成墙纸，没有视觉层级。
- v3 改为三条思路：尺度落差（三个词，字号 520/340/190px，超大字被视口裁切只露笔画）、深度分层（三层字加两条细线，各自不同视差速率）、动效绑定任务（入场错开淡入 + 滚动视差表达纵深，无空转漂移）。
- 定位到一项影响全站的环境事实：本机在系统层面关闭了动画（`ClientAreaAnimation=0`、`UserPreferencesMask` 未置 UI 效果位），Chrome 据此上报 `prefers-reduced-motion: reduce`。这意味着**全站所有动效对当前使用者都是不可见的**，不限于本样板。用户反馈的「没有动效」主要由此造成。
- 为使动效可被目视评判，在样板页加显式「动效预览」开关：默认态仍遵守系统设置（第 6 节要求不变），仅在用户主动点击后覆盖降级块。该逃生舱仅存在于样板页，未引入任何正式站文件。
- 修掉一处真实设计缺陷（与环境无关）：入场错开原用 `transition` 简写 + `transition-delay`，但 delay 对所有被过渡属性生效，导致视差 transform 也被延迟最多 480ms 才启动；同时给滚动视差套了 1100ms 缓动，等于底层滞后一秒才跟上滚动。已改为长写法，delay 与过渡只作用于 opacity，视差 transform 不设过渡、直接跟随滚动。

### Testing
- 视差公式隔离验证：绕开降级块后手动置 `--site-progress` 为 0 / .5 / 1，三层位移分别为 0 / -20.8 / -41.6px（far）、0 / -41.6 / -83.2px（mid）、0 / -67.8 / -135.6px（near），速率差 3.3 倍，纵深成立。
- 真实浏览器端到端验证：开启预览开关后滚动，`--site-progress` 实测 0 → 0.5001 → 0.9997，五层（三字两线）全部响应，跨全程位移 far -41.6 / mid -83.2 / near -135.6 / ruleA -57.8 / ruleB -106.7px。
- 实测值与设定值（-46 / -92 / -150）相差固定 0.904 倍，等于 cos(25.3°)。位移发生在 `rotate(--zzz-skew)` 之后的坐标系内，被斜切角度投影，非缺陷。
- 修复前后对照：修复前同样滚动条件下 near 层位移恒为 0（被 480ms delay 挡住）、far/mid 因 1100ms 缓动只走到 -16.2 / -4.2px；修复后三层即时同步响应。
- 降级验证：未点预览开关时（即系统默认态），`prefers-reduced-motion: reduce` 生效，三层 opacity 直接为 1、transform 只保留 `rotate()`、无视差、无入场动画，符合第 6 节「L3 直接抵达结果」与第 11 节要求。
- 未运行任何 npm 门禁：样板为独立预览文件，未接入正式站、未改动任何现有页面或资产。接入正式站前必须实跑 `test:ui` / `test:contrast` / `test:formal`，本轮结果不得复用为其已验证依据。

### Notes
- 改动文件：
  - `F:\hooxi-zzz\stitch-layer-sample.css` — 整体重写为 v3：删除 v1 的 `.stitch__rows` / `.stitch__row` 平铺结构，改为 `.stitch__word--far/mid/near` 三层加 `.stitch__rule--a/b` 两线；新增 `--depth` 驱动的视差 transform；入场过渡改长写法使 delay 只作用于 opacity；降级块内新增 `body.force-motion` 预览逃生舱。
  - `F:\hooxi-zzz\stitch-layer-sample.html` — 结构由 8 行平铺 div 改为三字两线；删除 v1 的水印生成脚本（含 `offsetWidth` 探针逻辑）；新增滚动进度 rAF 节流与可后挂的 `bindScroll`；新增动效预览按钮，仅在检测到系统 reduce 时显示。
  - `F:\hooxi-zzz\screenshots\stitch-v3-top.png`、`stitch-v3-motion-top.png`、`stitch-v3-motion-bottom.png` — v3 渲染与视差位移对照证据。
- 待用户裁定，本轮均未擅自决定：缝合层用固定层（每屏上深下淡、底层始终连续）还是改为随页滚动（严格复刻镜像，水印只在首屏）；`design.md` 第 15.2 节之后两段悬空工程笔记（`### Windows 上跑 Playwright 脚本的坑`、`### 不凭空造数据`，原属附录 A，删除脚本头去尾留）如何处置。
- 回滚：删除 `stitch-layer-sample.html`、`stitch-layer-sample.css` 及 `screenshots/stitch-*.png` 即可完全撤销样板，无任何依赖需要清理，正式站文件未被触碰。若只想退回 v1 平铺版，v1 内容未单独保留，需依本条记录重建。

## 2026-08-05 - Task: 开启系统动画设置并按反馈调整缝合层样板视觉

### What was done
- 定位并解除一个影响全站动效验收的环境级障碍：本机在系统层面关闭了动画效果，Chrome 据此上报 `prefers-reduced-motion: reduce`，页面按设计合同第 6 节降级、视差与入场动效被整个剥掉。这意味着此前对全站「没有动效」的观感，看到的是降级后的静态版本，而非动效未实现。已改为开启，Chrome 现报 `reduce: false`，视差无需任何临时开关即直接生效。
- 按反馈调整缝合层视觉：背景过淡与字号过大两项同时收敛。三层字号整体缩小约 45%（远层 520→280px 上限、中层 340→190px、近层 190→116px），明度提高约 1.7 倍（3.2%/3.6%/5.5% → 6%/6.3%/8.6%），细线目标明度定为 0.62。
- 修正上一版遗留的两处实现缺陷：入场揭示规则原先把三层字与两条细线一并设为全不透明，导致细线的设计明度被覆盖，已拆分为各层独立目标明度；动效预览开关按钮缺 `hidden` 初始属性，系统动画开启后本应自动隐藏却仍显示，已补齐。
- 同步样板页自述文案中的过期数值（明度区间由 3%–5.5% 改为 6%–8.6%），避免页面自己的说明与实现不一致。

### Testing
- 系统动画开启验证：以 `SPI_GETCLIENTAREAANIMATION` 的实际返回值为准（Chrome 的判断依据是该 API，不是直接读注册表），改动后返回 1。真实浏览器实测 `matchMedia("(prefers-reduced-motion: reduce)").matches = false`、`no-preference = true`。
- 视差实测：滚动进度 0 → 0.9997 全程，五层同时响应，纵向位移 far -41.6px / mid -83.2px / near -135.6px，两条细线 -57.8px / -106.7px，速率差 3.3 倍。实测值与设定深度相差固定 0.904 倍，等于 cos(25.3°)——位移发生在斜切旋转坐标系内被投影所致，非缺陷。
- 缺陷修复验证：细线 opacity 实测 0.62（修复前被入场规则覆盖为 1）；预览按钮 `hidden` 属性存在、计算样式 `display: none`（修复前为 block）。
- 视觉验证：真实浏览器 1280×900 截图两张（`screenshots/stitch-v5-top.png`、`stitch-v4-top.png`），确认背景明度足够可辨、字号落差成立、正文与卡片网格不被干扰。
- 排查过程中修正两处自身验证方法错误，记录以免重复：其一，用 `getBoundingClientRect()` 量旋转容器内元素尺寸会得到变换后的外接矩形（实测行高 41px 被读成 967px），必须改用 `offsetWidth`/`offsetHeight`；其二，浏览器缓存导致 DOM 收到改动前的旧版本，`navigate` 不强制重载，需加查询参数绕过，否则会把「改动未生效」误判为「改法错误」。
- 未运行 npm 门禁：本轮只改独立样板页与本机系统设置，未触碰任何正式站实现文件、资产或指纹纳管文件。样板未接入正式站，因此不存在可对应的实现门禁；若后续把缝合层并入正式页面，必须在那一轮实跑 `test:ui` / `test:contrast` / `test:formal`，本轮结果不得复用。

### Notes
- 改动文件：
  - `F:\hooxi-zzz\stitch-layer-sample.css` — 三层字号与明度按反馈调整；拆分入场揭示规则使细线保留独立目标明度；`transition` 简写改长写法，使错开延迟只作用于淡入、视差位移直接跟随滚动不设过渡。
  - `F:\hooxi-zzz\stitch-layer-sample.html` — 动效预览按钮补 `hidden` 初始属性；自述文案明度数值同步为实际值。
  - `F:\hooxi-zzz\tools\enable-system-animation.ps1` — 新增，开启当前用户动画设置并在结尾打印回滚指令；脚本内注明 BOOL 型设置在 `SystemParametersInfo` 的 GET 与 SET 之间参数传法不同（首版混用导致返回 True 但未落地）。
- 系统设置改动与回滚：作用范围为当前用户（HKCU），不涉及系统全局。改动前 `ClientAreaAnimation` 值不存在（此前误读为 0）、`MinAnimate = 0`；实际使动画生效的是 `MinAnimate` 由 0 改 1。回滚可用脚本结尾打印的指令，或在「设置 → 辅助功能 → 视觉效果 → 动画效果」关闭。
- 动效预览开关是样板专用逃生舱，正式站不得引入——在正式页面覆盖用户的无障碍设置会同时违反第 6 节与第 11 节。它仅用于在系统降级状态下目视评判动效本体，系统动画开启后自动隐藏。
- 回滚：删除 `stitch-layer-sample.html`、`stitch-layer-sample.css`、`tools/enable-system-animation.ps1` 与 `screenshots/stitch-*.png` 即可完全撤销样板，无依赖需清理。系统设置需按上一条单独回滚。

## 2026-08-05 - Task: 强化首页三类主力卡片在墨淡主题下的层次感

### What was done
- 解决首页卡片「贴在背景上、读不出边界」的问题。先量化了两条可选路径：靠表面明度分层走不通（近黑底上 stage→panel→panel-2→panel-3 四层挤在极窄区间内、每层间隔仅 1.16 倍，肉眼分不出，继续拉开会把弱文字对比度压到无障碍底线）；靠描边与投影分层空间充足（白边不透明度从 16% 提到 30% 可把边线对比度由 1.66 提到 2.69）。据此选定后者，全程未动任何表面色。
- 对首页三类主力卡片（入口路径卡、精选代理人卡、影像卷轴卡）统一补三件事：提高描边不透明度、加一道顶边内高光模拟受光、把原本几乎看不见的纯黑硬投影换成可辨的深色硬投影。原投影在近黑底上对比度仅 1.05，等于没画。
- 顺带确认了一件前提事实：首页实际运行在墨淡（ink-wash）主题分支，不是默认深色主题——主题脚本把墨淡硬编码为默认值。样式文件开头那批默认深色变量在首页从未生效。本轮据此只改墨淡分支，未触碰默认深色变量。
- 修复本轮自身引入的一处交互回归：新加的基础投影规则特异性高于动效文件里的按下反馈规则，会把三张卡「按下时投影收缩」的触觉反馈全部压掉。已在同一分支内补回按下态投影收缩。此项为自查发现，非用户报告。

### Testing
- 改动生效验证（真实浏览器读计算样式）：路径卡投影 `inset 0 1px 0 rgba(255,255,255,.14), 8px 8px 0 rgba(0,0,0,.72)`、描边 `rgba(232,236,241,.24)`；代理人卡与卷轴卡投影 `6px 6px`、描边 `rgba(232,236,241,.22)`。三条均确认落地。
- 版本号与实际送达一致性验证：抓页面上 `home-neon.css` 的实际 href 为 `?v=home-acts-r8`，并从已解析样式表中取回新增的按下态规则原文，确认改动确实随新版本号送达、未被缓存拦下。
- 按下反馈回归验证（真实鼠标按下取样）：路径卡 8px→3px、代理人卡 6px→3px、卷轴卡 6px→3px，三张全部按预期收缩。修复前三张均无收缩。
- 视觉核对：真实浏览器 1280×900 截图两张（`screenshots/home-r8-finder-cards.png`、`home-r8-finder.png`），确认描边、顶边高光与右下硬投影在近黑底上可辨，卡片轮廓成立。
- 验证方法上修正了两处自身错误，记录以免重复：其一，改动前判定「投影被 `!important` 压制」的来源结论有误——当时靠二分法怀疑到另一份主题文件，本轮改用按元素反查实际命中规则后，确认真正的压制者是动效文件里一条入场动画收尾规则，与那份主题文件无关；其二，测交互伪类时把探针挂在 `window` 上取不到值，因脚本求值运行在隔离环境、不跨调用保留，须把取样结果写入 DOM 属性才能读回。
- 未运行 npm 门禁：本轮只改首页主题分支的描边与投影表现，未触碰数据、脚本逻辑或资产。若后续把同一手法推广到子页面，需在那一轮实跑 `test:ui` / `test:contrast`，本轮结果不得复用。

### Notes
- 改动文件：
  - `F:\hooxi-zzz\home-neon.css` — 墨淡分支内三条卡片规则各补描边不透明度、顶边内高光与可辨硬投影；新增一条按下态规则，把三类卡片按下时的投影收缩补回（因新增基础投影特异性高于动效文件的按下反馈）。表面色与面板变量未改。
  - `F:\hooxi-zzz\index.html` — 首页引用的 `home-neon.css` 版本号由 `home-acts-r5` 改为 `home-acts-r8`，用于让改动绕过浏览器缓存。
  - `F:\hooxi-zzz\screenshots\home-r8-*.png`、`home-r7-*.png`、`home-dark-audit-*.png` — 改动前后视觉核对证据，仅供人工比对，可随时删除。
- 本轮发现但未修的既存问题（均不在本轮范围内，未擅自改动）：一是首页卡片的悬停上浮从未生效，被动效文件里一条针对入场揭示元素的 `transform:none!important` 压死，而所有悬停上浮规则都没有 `!important`，首页三张路径卡都带该揭示标记；二是路径卡的大号水印数字压在小标签上、标题受宽度限制被切成断行；三是右侧楼层索引条压在卷轴卡上。
- 回滚：撤销上述两个文件的本轮改动即可，无依赖需清理。最小回滚为把 `home-neon.css` 墨淡分支内新增的按下态规则整行删除、三条卡片规则去掉本轮新增的 `box-shadow` 与描边不透明度提升，并把 `index.html` 的版本号改回 `home-acts-r5`。两文件均在本轮之前即处于未提交状态，无法用单条 git 命令定点回退，须按本条描述手改。

## 2026-08-05 - Task: 用胶片片边处理首页主视觉上下留白

### What was done
- 解决首页首屏主视觉上下留白（俗称黑边）的观感问题。先确认了留白是几何必然、不是实现缺陷：主视觉源图只有 2200×928（约 21:9）一个比例，而首屏容器被拉到满屏高（实测 1280×900，比例 1.42），按完整显示（contain）摆放后上下各空 180px，占容器高度 40%。
- 上一轮已实测排除两条「把图铺满」的路径，本轮不再重试：改为裁切填充（cover）会横向切掉 482px、只剩中间 58% 画面，多人构图变成单人头部特写、帽顶被切；只让远景层单独铺满则因远景层是抠掉主体后的背景板（带修补空洞），放大后与中近景层脱开对位，构图断裂。换图不可行，源图无其他比例。
- 据此按用户选定方案施工：不裁画面、不改容器比例，在画面上下边缘各加一条胶片齿孔带，把留白读作「一格胶片的片基」而不是「图没铺满」。纯 CSS 渐变实现，未加任何图片资源、未改 HTML 结构、未动视差脚本。
- 施工中发现并绕开一个几何前提，否则会做出错位效果：主视觉容器带一个约 3° 的既有静态旋转（GSAP 通道内联写入，实测 5 次采样恒为 `rotate(3.00023deg)`、非动画残留）。因此齿孔带必须留在旋转容器内部才能与画面边缘保持平行；若挂在未旋转的外层，齿孔会与倾斜的画面边缘呈夹角。
- 修正了本轮自身引入的一处视觉回归：首版把片基做成比底色亮一档的渐变，结果把容器旋转后四角未覆盖的楔形区暴露成一道明显的亮色斜向接缝（此前底色接近纯黑故看不见）。已改为片基色贴近底色，接缝消失。此项为自查截图发现，非用户报告。
- 齿孔带定位不用固定像素，改用「百分比 padding 参照容器宽度」的特性锚定：完整显示后的图高恰好等于容器宽除以图比例，所以 `padding-bottom:42.182%` 得到的盒高即等于画面高度，两条齿孔带因此恒定贴住画面上下边缘，不随视口高度变化错位。

### Testing
- 几何对齐验算（真实浏览器读实际盒模型）：容器布局 1280×900，完整显示后图高 540、上下留白各 180。定位盒实测 `padding-bottom:607.922px`、盒顶 146、盒底 754；盒顶 146+34 条宽=180=画面顶边，盒底 754−34=720=画面底边，两端均精确贴合。
- 视差露边验证（本轮不可省项，改动的正是画面边缘）：把指针推到左上、右下两个极限位并等缓动稳定后取样，三层最大纵向位移 far ±1px / mid ±2.5px / near ±5.4px。远景层作为兜底层（最底层、放大倍率最高、位移最小）其画面边缘恒定落在齿孔带覆盖区内，因此近景层被推到最大位移时露出的是下层画面而非背景空隙。34px 条宽亦足以盖住三层放大产生的约 19px 溢出。
- 视觉核对：真实浏览器 1280×900 截图三张（`screenshots/hero-film-r1.png` 首版含接缝、`hero-film-r2.png` 修正后、`hero-film-r2-parallax-br.png` 视差极限位）。确认齿孔带随 3° 倾斜与画面边缘平行、无错位；上方齿孔带不被顶栏遮挡；文案卡按既有层级正常压在下方齿孔带之上；亮色斜向接缝已消失；两端极限位均无露边。
- 排查过程中修正一处自身判断错误，记录以免重复：上一轮按容器外接矩形（1325×966）算出留白 203px，该数值偏大。旋转元素的 `getBoundingClientRect()` 返回的是变换后外接矩形，量布局尺寸须改用 `offsetWidth`/`offsetHeight`，实测布局尺寸为 1280×900、留白应为 180px。本轮所有几何均按修正后数值计算。
- 3° 旋转的写入源未定位到：已排除样式文件（无 `rotate:` 声明与对应 transform 规则）、视差脚本（只对三个图层写 transform，未碰容器）、滚动动效脚本（只写 scale）、布局编辑器（只写 `--layout-x/y/w/z` 四个 CSS 变量，不写 transform）、已发布布局与本机存档（均为空）、首页内联脚本。因该旋转为既有静态效果且本轮修法已适配它，未继续追查、未改动它。
- 未运行 npm 门禁：本轮只改首页首屏主视觉的装饰层表现，未触碰数据、脚本逻辑、资产或无障碍相关实现。

### Notes
- 改动文件：
  - `F:\hooxi-zzz\zzz-hero-parallax.css` — 文件末尾新增一条胶片片边规则（`.hero-carve::before`），用百分比 padding 锚定画面高度，以多层渐变画出上下两条齿孔带、片门线与不透明片基底。原有的层样式、视差层、氛围光、粒子层与移动端规则均未改动。
  - `F:\hooxi-zzz\index.html` — 首页引用的 `zzz-hero-parallax.css` 版本号由 `zzz-carve-r4` 改为 `zzz-carve-r6`（中途经 r5 一版），用于让改动绕过浏览器缓存。
  - `F:\hooxi-zzz\screenshots\hero-film-r1.png`、`hero-film-r2.png`、`hero-film-r2-parallax-br.png` — 改动前后与视差极限位的视觉核对证据，仅供人工比对，可随时删除。
- 齿孔带的定位数值 `42.182%` 与源图比例 2200×928 绑定。若后续更换主视觉且新图比例不同，该数值必须同步改为「图高÷图宽」，否则齿孔带会脱离画面边缘。此约束已写入该规则的注释内。
- 本轮发现但未修的既存问题（均不在本轮范围内，未擅自改动）：一是主视觉容器那 3° 旋转的写入源无法在源码中定位（排除过程见上），它同时导致容器四角有未覆盖的楔形区，目前因与底色同色而不可见；二是留白占比在窄屏竖向视口下会进一步放大，本轮片边方案只改善观感、不改变留白比例本身。
- 回滚：把 `zzz-hero-parallax.css` 末尾那条 `.hero-carve::before` 胶片片边规则连注释整段删除，并把 `index.html` 的版本号改回 `zzz-carve-r4` 即可，无依赖需清理。注意 `zzz-hero-parallax.css` 在本轮之前即有大量未提交改动（整个卡雕视差重写），不可用 `git checkout` 该文件回退，否则会连带撤销非本轮内容，须按本条描述定点手改。

## 2026-08-05 - Task: 修复主线剧情页列表整体空白（57 条结果但一条都看不到）

### What was done
- 修复主线剧情页的可用性事故：页面顶部正常显示「57 / 57 条结果」，但下方列表区域整体空白，用户看不到任何一条内容。修复后 57 条全部正常显示。
- 根因不是数据缺失、不是筛选逻辑、也不是样式覆盖，而是入场动效的触发条件在这一页数学上永远不成立。子页共用的滚动入场动效要求元素「露出自身高度的 8% 才淡入」，而这一页把 57 条内容装在一个整体容器里，该容器高 13849px，8% 即需露出 1108px；可视区只有 840px（已扣除动效自身的 60px 底部余量），条件恒不满足，整个容器永久停在完全透明状态，连带里面所有内容一起看不见。
- 修法取最小改动：把触发条件从「露出 8%」改为「露出即触发」，一处数值。入场动画本身、动画时长、错落节奏、视差与标题闪烁均未改动，观感不变，只是超高容器不再被卡住。
- 同步补上缓存版本号。该动效脚本被 6 个子页共用且都写死同一个版本号，只改脚本内容不升版本号的话，线上回访用户浏览器会继续用旧缓存，这一页对他们仍然是空白。按本项目既有惯例把 6 页统一升版本，确保修复能真正送达。

### Testing
- 修复前根因取证（真实浏览器实测，非推算）：读到承载 57 条内容的容器实际高度 13849px、可视区高度 900px，代入原触发条件得出所需露出量 1108px > 840px，确认条件恒不成立；同时读到该容器透明度恒为 0、始终未拿到入场标记，与推断一致。
- 修复后主线页验证：1280×900 真实浏览器逐屏平滑滚动到底，57 条内容全部拿到入场标记、全部可见、全部带封面图，两个分组容器透明度均为 1，页面计数显示「57 / 57 条结果」。另存 900px 截图一张作视觉核对，确认标题、筛选栏、分组标题、卡片封面/版本号/标题/摘要/外链全部正常呈现。
- 首屏单独验证（不滚动只等待）：第一条内容自行入场、透明度 1，确认修复对首屏可见元素同样生效。
- 其余 5 个共用页回归验证：最复杂的往期活动页（262 个入场元素、15 个分组）前台实测 15 组全部正常入场；页内剩余未显示的条目经逐项核查属折叠分组内的内容，展开后即正常入场（实测一组从 62px 展开到 6383px、组内 32 条有 18 条立即入场，其余仅因未滚到），属设计行为而非缺陷。其余页面无同类超高容器结构。
- 版本号送达复验：改版本号后重新加载主线页，确认页面实际引用的是新版本、且交付到浏览器的脚本内容确为修改后的值，57 条仍全部正常。
- 验证方法上修正了两处自身错误，记录以免重复：其一，用后台新标签页批量测 5 个页面得出的「多组隐藏」结论不可信，浏览器会节流后台标签页的定时器与可见性回调，改为前台逐页导航后同一批数据即恢复正常；其二，一度把「滚到底后仍隐藏的第一条」判为修复不彻底，实为测试路径问题——滚到底后未回顶，该元素在动效初始化后再没进过可视区，回顶复测即正常。
- 未运行 npm 门禁：本轮只改一处动效触发阈值与 6 处缓存版本号，未触碰数据、页面结构、样式或无障碍实现。

### Notes
- 改动文件：
  - `F:\hooxi-zzz\subpage-motion.js` — 滚动入场观察器的触发阈值由 `0.08` 改为 `0`（第 37 行），使超高容器不再因需露出比可视区更多的高度而永久卡在透明状态。观察器的底部余量、错落延迟、视差与标题闪烁逻辑均未改动。
  - `F:\hooxi-zzz\mainline.html`、`events.html`、`behind-scenes.html`、`cultivate.html`、`character.html`、`faction.html` — 各改一行，引用该动效脚本的缓存版本号由 `sp-1` 改为 `sp-2`，用于让修复绕过浏览器缓存送达回访用户。6 个文件除这一行外本轮未做任何其他改动。
  - `F:\hooxi-zzz\artifacts\mainline-fixed-900px.png` — 修复后 900px 视口视觉核对证据，仅供人工比对，可随时删除。
- 回滚：把 `subpage-motion.js` 第 37 行的 `threshold: 0` 改回 `threshold: 0.08`，并把上述 6 个 HTML 中的 `subpage-motion.js?v=sp-2` 改回 `?v=sp-1` 即可，无依赖需清理。注意两点：`subpage-motion.js` 是未跟踪文件，git 无法回退，只能手改；6 个 HTML 在本轮之前即有大量未提交改动（合计逾 2000 行新增），绝不可用 `git checkout` 回退这些文件，否则会连带撤销非本轮内容。
- 遗留约束：该动效脚本为 6 个子页共用，后续若再调整这处阈值，须同时考虑最高容器的高度是否仍小于可视区，否则会重现同类整块空白。

## 2026-08-05 - Task: 修复首页精选代理人区被迫横滚与楼层标题折行

### What was done
- 修复首页两处确证的呈现缺陷，均只改样式，未动数据与脚本逻辑。
- 精选代理人区原本是横向滑轨：8 张卡按角色名长短各自撑开成 316~430px，总宽 3068px 挤在 1100px 容器里，用户不横滚只能看到 2 张完整卡片，其余 6 张全在视野之外。改回本就存在的 4 列等宽网格后，8 张分两行全部可见，无需任何横向操作。该滑轨原为有意设计（注释写明「出血段内卡片横向溢出滚动」），但在当前的卡片数量与角色名长度下会把大部分内容藏起来，故按可用性优先移除，并把原意图与移除理由留在原处注释里。
- 三个楼层标题（直接查档 / 精选代理人 / 档案卷轴）原本被挤在窄列内折成两行。原因是楼层头部为两列网格，标题按 DOM 顺序自动落进右侧那列注释列，实测该列只有 310.5px，而 64px 字号的 5 字标题需约 320px，必然折行。显式让标题占第一列后得 846~866px，一行排开；注释自动流入窄列，其本身 293px 的宽度上限正与窄列匹配。
- 同步升两处样式的缓存版本号，确保回访用户拿到新样式而不是旧缓存。

### Testing
- 真实浏览器 1280×900 前台实测，逐屏平滑滚动让入场动效走完后再取样，避免瞬跳导致的漏触发假象；量布局尺寸统一用 `offsetWidth/Height`，不用会受变换影响的外接矩形。
- 代理人区修复后：容器实际布局为网格、四列各 264.5px，`scrollWidth` 与 `clientWidth` 同为 1100px、无横向滚动条；8 张卡分两行（每行 4 张）全部入场、透明度均为 1。
- 缺陷复现取证（不靠推算）：在同一页面临时注入原横向滑轨声明后重测，`scrollWidth` 3068px、卡片宽 316~430px、完整可见仅 2 张、确有横向滚动条；随即恢复网格，`scrollWidth` 回到 1100px。注入与恢复在同一会话内成对完成，未在文件中留下改动。
- 标题修复后：三处楼层标题实测均为单行（高 63px、字号 64px），占第一列后分别得 846px / 866px / 866px；三处注释均正常落在 293px 窄列内。
- 版本号送达复验：重新加载后页面实际引用的样式确为 `home-acts-r9` 与 `zzz-motion-r7`。
- 响应式回归：移除 flex 覆盖后，880px 档 3 列、640px 档 2 列、更窄档的间距收窄规则恢复生效（此前均被 flex 压掉）；640px 档楼层头部本就是单列网格，本轮新增的占列规则在该档无副作用。
- 视觉核对：代理人区 900px 截图一张，确认两行八卡完整呈现、编号角标与楼层导航条无错位、标题单行。
- 未运行 npm 门禁：本轮只改两条样式规则与两处缓存版本号，未触碰数据、脚本逻辑或无障碍实现。

### Notes
- 改动文件：
  - `F:\hooxi-zzz\zzz-motion.css` — 移除精选代理人区的横向滑轨覆盖，该处只保留 12px 底部留白，并把原设计意图与移除理由写进注释。同文件其余动效规则未改动。
  - `F:\hooxi-zzz\home-neon.css` — 新增一条规则让楼层标题显式占第一列，修正折行。相邻的标题字号、行高、字距规则未改动。
  - `F:\hooxi-zzz\index.html` — 两处样式引用的缓存版本号：`home-neon.css` 由 `home-acts-r8` 改为 `home-acts-r9`，`zzz-motion.css` 由 `zzz-motion-r6` 改为 `zzz-motion-r7`。
  - `F:\hooxi-zzz\artifacts\home-r9-agents-grid.png` — 代理人区修复后视觉核对证据，仅供人工比对，可随时删除。
- 本轮取舍：卡片宽度由原滑轨的 316~430px 收到 265px，长角色名折行更多（最长者折 3 行），摘要仍按既有两行截断规则收尾。相比原先 6 张卡完全看不到，判断可见性优先。代理人区由单行 150px 变为两行 410px，页面总高由 6377px 增至 6655px。
- 修正一处此前的自身判断错误，记录以免重复：一度按 4 张精选卡描述该区并据此写了注释，实测为 8 张（两行各 4 张），注释已改正。相关的宽度与可见数量结论也已按 8 张重算。
- 回滚：把 `zzz-motion.css` 那段注释连同 `padding-bottom` 规则替换回原横向滑轨声明、删除 `home-neon.css` 中新增的占列规则、并把 `index.html` 两处版本号改回 `home-acts-r8` 与 `zzz-motion-r6` 即可。注意两点：原滑轨规则的完整声明内容已无法从 git 取回（其所在的整段动效增强均为未提交新增），只知其核心为横向排布加子项不收缩加横向溢出滚动，若确需恢复横向滑轨建议重新设计而非照抄；这三个文件在本轮之前即有大量未提交改动，绝不可用 `git checkout` 回退，须按本条描述定点手改。

## 2026-08-06 - Task: 修复角色档案数据文件语法错误与首页卡雕三层配准错位

### What was done
- 恢复角色档案数据在线可用。该数据文件此前整体无法被浏览器解析，等于全站 57 名代理人的简介、称号、图集数据全部失效。根因是新补的蕾米埃尔简介里有三对中文强调引号用了未转义的半角双引号，把数据字符串提前截断。只补转义、不改显示字符，档案原文保真。修复后 57 名角色全部正常解析，蕾米埃尔的简介、本地图标、图集条目均可读出。
- 修正首页卡雕三层的配准错位。三层图叠放时同一处内容在各层落点不一致，静止态最大偏差 10.52px，而鼠标视差想表达的纵深最大只有约 22px，等于一半的层间位移是无意义的固定噪声，观感上就是轮廓发虚、边缘不服帖。三层各自的缩放值经透视放大后并不相等，反解 translateZ 的透视放大量后使三层有效缩放一致。
- 修正呼吸动画在运行中重新引入错位的问题。上一轮只对齐了起始态，画面动起来后偏差回到 14.2px，原因是三层缩放动画时长各不相同（9 秒 / 7 秒 / 4.5 秒），同一时刻各层走过的比例不同，有效缩放又被拉开。把缩放从呼吸动画中拆成三层同时长同缓动的独立部分，微旋与微位移仍保留原有的不同步节奏，Live2D 感的来源未被削弱。
- 同步减少动效路径的缩放值，使关闭动效的访客与常规访客看到同一套对齐结果。

### Testing
- 数据文件语法：`node --check` 通过；实际载入后遍历得 57 名角色，逐字段核对蕾米埃尔的简介全文、称号、本地图标路径与图集条目均正确，三处中文引号在解析结果中正常显示。
- 卡雕配准：真实浏览器读取三层的实际渲染变换矩阵，按 `perspective 1000px` 折算各层有效缩放，取画面角点半径换算成像素偏差，连续采样约 90 帧覆盖完整呼吸周期。静止态由 10.52px 降到 0.037px；动态全程最大由 14.219px 降到 0.082px。
- 修复方向取证不靠推算：在浏览器内临时重建三种呼吸动画方案对比，确认"统一缩放时长"是唯一起作用的变量（现状 14.219px、统一时长 0.980px、完全去掉缩放呼吸 0.055px），三种方案的旋转差与平移差完全相同，排除了旋转与位移是错位来源的可能。据此选择保留缩放呼吸并统一时长，而非牺牲呼吸效果。写入源码时按透视反解取精确峰值，把统一时长方案的残留 0.980px 进一步压到 0.082px。
- 内容门禁 `npm run test:content`：1 项失败（剧情页稳定来源节点缺来源声明）。已用独立 worktree 拉干净的已提交版本复测，该版本通过，据此确认失败来自剧情页的未提交改动，与本轮无关。
- 可读性门禁 `npm run test:contrast`：失败。其中 30 秒超时中止在干净已提交版本上同样出现，属既存问题。另有三项对比度数值回退（4.22:1 / 3.85:1 / 4.25:1，门槛 4.5:1），干净版本对应数值为 19.21:1 / 17.8:1 / 12.67:1。已在 worktree 中按文件二分定位到单一来源：主题样式文件的未提交改动中，有一条把剧情页舞台元数据文字设为主题色 70% 透明度的规则。该规则不在本轮改动范围内，未擅自修改。
- 未运行 `test:stories` 与 `test:ui`：本轮未触碰剧情页构建产物与基线截图相关实现。

### Notes
- 改动文件：
  - `F:\hooxi-zzz\agent-enrichment.js` — 为蕾米埃尔简介中三对中文强调引号补上转义，使整个数据文件恢复可解析。同文件其余 56 名角色数据未改动。
  - `F:\hooxi-zzz\zzz-hero-parallax.js` — 三层起始缩放改为透视反解后的等效值；呼吸动画中的缩放拆为三层同时长同缓动，微旋与微位移保持原有不同步节奏；就近补注释说明时长必须统一的原因。视差、事件绑定、减少动效分支等其余逻辑未改动。
  - `F:\hooxi-zzz\zzz-hero-parallax.css` — 三层缩放值与脚本同步（减少动效路径）；文件头部注释原本写的 `1.10 / 1.06 / 1.03` 与代码实际值本就不符，因该注释正是描述本轮改动的这三个值、且新值小于 1 反直觉容易被误当笔误改回，故一并更正并说明原因。其余层样式、投影、粒子等规则未改动。
- 本轮修正的自身判断错误，记录以免重复：上一轮结论称"改三个缩放数字即可"，实测证明不足——只改数值只能治静止态，动态帧偏差几乎回到修复前水平，真正的病根是缩放动画时长不同步。另外上一轮曾预告"对齐后近景投影会沿切割线露暗边"，静止帧与动态采样均未出现，该预告不成立。
- 回滚：
  - 数据文件：把蕾米埃尔简介中三处 `\"异化\"`、`\"虚耀\"`、`\"耀变\"` 的反斜杠去掉即可回到本轮之前状态，但该状态存在语法错误、整个文件在浏览器中失效，不建议回滚。不可用 `git checkout` 回退此文件，否则会连带撤销非本轮的内容工作（蕾米埃尔简介全文、本地图标替换外链、图集条目）。
  - 卡雕脚本：三层起始缩放改回 `1.05 / 1.03 / 1.02`；把拆开的缩放部分与微旋微位移合回三条，即远景 `scale:1.07, rotation:-0.15, x:3, y:2, duration:9`、中景 `scale:1.05, rotation:0.2, x:-3, y:2, duration:7, ease:power1.inOut`、近景 `scale:1.04, rotation:-0.1, x:2, y:-3, duration:4.5, ease:back.inOut(1.2)`。
  - 卡雕样式：三层缩放改回 `1.05 / 1.03 / 1.02`，头部注释改回 `1.10 / 1.06 / 1.03`。
  - 这两个卡雕文件在本轮之前即有大量未提交改动，绝不可用 `git checkout` 回退，须按上述数值定点手改。

## 2026-08-06 - Task: 补齐诊断截图忽略规则并把当前视觉状态推到审阅分支

### What was done
- 按用户"先上传到 git 让别人找问题"的要求，把当前在建状态推到远端新分支 `review/visual-state-20260806`，供他人拉取审阅。未推 `main`。
- 选择新分支而非 `main` 的原因是安全边界：仓库存在 `.github/workflows/pages.yml`，触发条件为 `push: branches: [main]`，推 `main` 会直接触发 GitHub Pages 线上部署。用户要的是"给人审"，不是发版，两者必须分开。`main` 全程未被触碰，推送前后均为 `cb18a649`。
- 给忽略清单补上本机诊断截图目录规则。该目录 36 张大截图共 19.3MB，只服务于本机比对，此前漏写规则，不补会随本次提交进入仓库、让仓库体积无谓翻倍。规则补在忽略清单中既有的"本机工作产物，不入库"意图块内，与相邻的根目录截图、参考素材、源码打包三条同属一类，未新建分区、未改动其他规则。
- 提交范围做了三项显式排除：诊断截图目录、一个 686KB 的历史进度临时文件、两个探针与设计文档备份文件。三者均非站点资产，入库会让审阅者误认为是正式内容。排除用路径规则精确指定，未使用全量暂存兜底。
- 本轮之前修复的两处内容随本次提交一并入库：角色数据文件的引号转义修复（未修时整个文件语法失效、57 人档案全部读不出）、卡雕三层错位修复。角色数据修复所依赖的两个图片资源此前均为未跟踪状态，已确认随本次提交一并入库，否则线上会出现图片失效。

### Testing
- 忽略规则生效验证：补规则后单独查验诊断截图目录的忽略判定，结果为已忽略；补规则前同一查验为未忽略、会被提交。
- 行尾未被破坏验证：一度担心编辑动作把忽略清单整体转成了另一种行尾格式，实测该文件工作区与已提交版本的回车符计数同为 0，本来就是单一行尾，差异统计为纯 3 行新增、无格式改写。仓库对 34 个文件普遍提示行尾转换警告，属既有配置行为，非本轮引入。
- 暂存内容核对：暂存 161 个文件，其中新增 125 个共 24.61 MB；逐项确认诊断截图目录、三个临时文件、临时产物目录均未误入。
- 密钥风险排查：按文件名关键词扫描未跟踪与已改文件，两处命中均为设计变量文件（色板 token 与全站 token），实际内容为样式变量、非凭据；另按常见凭据字面模式扫描待提交内容，无命中。
- 大文件溯源：暂存中两个较大文件逐一查引用，21:9 预览图确被本次同批新增的一个演示页引用故保留；另有一个 2.5MB 动图与一张检查用图确认全站无引用，按规范只指出、未删除他人产物。
- 推送后状态核对：远端已出现该分支且与本地同步；工作区仅剩三个被显式排除的临时文件；`main` 与 `origin/main` 均仍为推送前的提交。
- 未运行 UI 与剧情两项测试。内容与链接校验此前已跑，其中两项失败经干净基线逐组二分确认为未提交在建工作引入、非本轮改动所致，详见下方遗留说明。

### Notes
- 改动文件：
  - `F:\hooxi-zzz\.gitignore` — 在既有"本机工作产物，不入库"块内新增诊断截图目录忽略规则一条并附两行说明，其余规则未动。
- 本轮同时产生一次提交与一次推送，提交为 `9e16af6`，含 161 个文件；该提交同时携带此前多个会话的在建改动（合计 32 个已改文件与 125 个新增文件），并非全部为本轮工作，审阅时需注意区分。
- 遗留待你决定的两处回退，均非本轮改动，已定位到单一来源但未擅自修改：一是剧情页缺一处不可减负的来源声明，卡住仓库自带的边界门禁，涉及版权合规文案；二是色板文件新增样式把一处元数据文字压到七成透明度，可读性从 19.21 掉到 4.16、低于无障碍标准线，属另一会话的设计选择。建议在通知他人审阅时一并说明这两项为已知项，避免重复上报。
- 三个被排除的临时文件仍留在工作区未入库，如需归档请自行处理；它们不影响站点运行。
- 回滚：删除忽略清单中新增的那条诊断截图目录规则及其两行说明即可恢复原状。远端分支如需撤回，删除远端同名分支并删除本地分支即可，`main` 与线上发布不受影响，因本次全程未推 `main`。本进度记录本身在提交之后追加，故不在该提交内。

## 2026-08-06 - Task: 修复首页楼层标题重复与查档卡标题折行

### What was done
- 移除首页楼层导航里重复的小号标题。该处原本会把楼层大标题的文字复制一份，以小字号追加到导航块中，意图是复刻官方站「中文 + 英文 + 编号」三行结构。但本站每个楼层的大标题本身就以 64px 显示同一句中文，小标签成了纯重复，读者在同一屏内看到同一个词两次。实测确认 4 个带导航的楼层文字全部相同、大标题全部可见，故移除标签生成，保留英文名与编号两行。
- 修正首页三张查档卡标题的折行。原先「角色与阵营」「按名字找人」这类五字标题会断成两行，末行只剩一个字。根因不是宽度设得太小，而是宽度单位选错：原规则用 `9ch`，`ch` 是数字「0」的宽度（实测 17.4px），中文字符在该字号下每字约 38.4px，因此「九个字符」的额度实际只装得下四个中文字。改用 `em` 后额度与中文字宽匹配，标题恢复单行。
- 同步升高受影响样式与脚本的缓存版本号，确保回访用户拿到新文件而非旧缓存。

### Testing
- 楼层标题：真实浏览器实测五个楼层，修复前四个楼层各存在一个与大标题文字完全相同的小标签，修复后小标签 0 处、五个大标题全部仍可见且字号未变；英文名与两位编号未受影响，标题位置上移 26px，正好等于被删掉的一行高度加行间距。
- 折行修复前先做单位验证，不凭推断动手：在同一页面内分别试 `9ch`、`9em`、无限制、`6em` 四种取值，`9ch` 折两行、其余三种均单行，确认问题出在单位而非数值。
- 四个断点复验（1280、1440、880、390）：三张卡标题全部单行，残留小标签 0，五个大标题在四档下均可见。卡片高度随之降低（341→304、309→272、277→240），页面总高由 6640 降至 6339。
- 视觉核对：桌面与移动各两张截图，确认两处修复同时生效且版式未塌陷。
- 门禁：档案媒体校验 17 组全通过、链接诚信检查通过。非官方边界检查有 1 项失败（`stories.html` 缺来源声明），该项为本轮之前即存在的未提交改动引入，已用干净基线 worktree 单独复现确认与本轮无关，本轮未触碰该文件。

### Notes
- 改动文件：
  - `F:\hooxi-zzz\zzz-official.js` — 移除楼层小标签的生成逻辑，并清理因此不再被使用的两个局部变量；同步更新该函数上方与文件头两处描述「三行结构」的注释。同文件的角色页缩略图条与琴键缩放逻辑未改动。
  - `F:\hooxi-zzz\official-dna.css` — 删除小标签的样式规则（其作用对象已不存在），并把该段注释里的「三行」改为「两行」。
  - `F:\hooxi-zzz\home-neon.css` — 查档卡标题的宽度上限由 `9ch` 改为 `9em`，同行其余声明未动。相邻的正文段落仍保留 `30ch`，其两行呈现是正常收尾，未改。
  - `F:\hooxi-zzz\index.html` — 三处缓存版本号：`official-dna.css` 与 `zzz-official.js` 由 `r1` 升至 `r2`，`home-neon.css` 由 `home-acts-r9` 升至 `home-acts-r10`。
  - `F:\hooxi-zzz\character.html` — 两处缓存版本号同步升至 `r2`。该页也加载这两个文件，浏览器按地址缓存，不升则该页仍取旧文件；该页本身无楼层导航结构，删除的样式规则对它无影响。
  - `F:\hooxi-zzz\artifacts\fix2-d.jpg`、`fix2-m.jpg`、`fix-head-d.jpg`、`fix-head-m.jpg` — 视觉核对证据，仅供人工比对，可随时删除。
- 本轮修正两处此前基于截图的错误判断，记录以免重复：一是曾把「主视觉与首个楼层之间有约 430px 空荡区」列为缺陷，实测该处仅 121px，且上方元素为滚动固定占位，截图里的空白是固定滚动过程中的中间态，属设计意图，该条建议作废；二是曾描述楼层右侧有「超大编号溢出裁切」，实测全页无 80px 以上字号元素，编号为 38px 且位于左侧，该描述不成立。
- 指出但未改动的既存项：`zzz-official.js` 中的 `pad` 函数零使用点，是本轮之前即存在的死代码，按范围约束未删。
- 回滚：
  - 楼层小标签：在生成编号的代码块之后补回「读取大标题文字、生成 `section-nav-label` 小标签并追加到导航块」的逻辑，并在 `official-dna.css` 中补回该类的字体族、字重、字距三条声明。
  - 卡标题：把 `home-neon.css` 中该条的 `9em` 改回 `9ch`。
  - 缓存版本号：`index.html` 三处与 `character.html` 两处按上述对应关系改回。
  - 这五个文件在本轮之前均有未提交改动，绝不可用 `git checkout` 回退，须按上述描述定点手改。本轮改动已提交至审阅分支，也可用该提交的反向补丁精确撤销。
