(()=>{
  ...
  document.querySelector('#characterName').innerHTML=`${esc(character.name)}<br/><span>角色档案</span>`;
  ...
  document.querySelector('#characterPortrait').innerHTML=character.portrait?`<img src="${esc(character.portrait)}" alt="${esc(character.name)} 立绘" loading="lazy"/>`:'<span>立绘待接入</span>';
