...
```bash
# 1. 配置环境变量（复制 .env.example 为 .env 并填写真实值）
...
浏览器打开 `editor.html`（如 `http://localhost:8080/editor.html`），使用三账号中的任一账号登录。

### 编辑流程（编辑员）

1. 登录后选择 `data.js` 或 `layout-data.js`
2. 在 JSON 编辑器中修改结构化内容
3. 点击"保存到本机"写入浏览器 localStorage 草稿（Ctrl+S 快捷键）
4. 确认无误后，点击"提交检阅分支"
5. 后端自动创建 `review/draft-<timestamp>` 分支并推送文件

### 发布流程（管理员）

1. 管理员登录后，左侧"检阅"区列出所有待审阅分支
2. 点击分支名可查看文件内容（需点击"检阅文件"按钮）
3. 审查通过后，点击"发布"按钮将内容写入 `main` 分支
...
```
GITHUB_TOKEN=ghp_xxxxxxxxxxxx    # 细粒度 PAT，需 Contents 读写权限
GITHUB_REPO_OWNER=pokkan39
GITHUB_REPO_NAME=hooxi-zzz
```

未配置 GitHub 时，编辑器仍可使用本地草稿保存/加载功能，但检阅推送和发布操作不可用。
