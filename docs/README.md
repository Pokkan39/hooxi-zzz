...
3. 选择 `data.js`，默认用“可视化编辑”表单修改阵营、角色、主线、角色故事、往期活动和幕后/对谈。
4. 需要直接改文件时切到“源码编辑”；`layout-data.js` 暂时只支持源码编辑。
5. 点击“保存到本机”可把草稿写入当前浏览器；这一步不会影响线上访客。
6. 确认无误后点击“导出当前文件”，用下载文件覆盖仓库根目录的同名文件。
7. 执行 `git add`、`git commit`、`git push origin main`，等待 GitHub Pages 或绑定域名的托管服务更新。
...
密码通过后可编辑 `data.js` 和 `layout-data.js`。`data.js` 默认提供可视化表单，并会自动同步到源码；`layout-data.js` 暂时只支持源码模式。编辑器只会保存到当前浏览器或导出文件，不会直接改线上站点。
